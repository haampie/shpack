...
-DTCC_VERSION=\"0.9.26\" \
-DONE_SOURCE=1 -o tcc.sl ../src/stdlib.c ../tcc_sources/tcc.c
echo stack_c
../src/stack_c -i ../src/stack_c_intro.M1 tcc.sl -o tcc_s.M1
echo bloof-elf
../src/blood-elf --file tcc_s.M1 --little-endian --output tcc_s.blood_elf
echo M1
../src/M1 tcc_s.M1 -o tcc_s.macro
echo hex2
../src/hex2 -o tcc_s ../M2libc/x86/ELF-x86-debug.hex2 tcc_s.macro tcc_s.blood_elf
...
# Create config.h
${ORG_STAGE0}/catm ${MES_PKG}/include/mes/config.h
${ORG_STAGE0}/catm ${TCC_PKG}/config.h
...
if ${ORG_STAGE0}/match ${ARCH} amd64; then
    MES_ARCH=x86_64
...
${TTC_G_BOOT} -c -D HAVE_CONFIG_H=1 -I include -I include/linux/${MES_ARCH} -o ${LIBDIR}/crt1_g.o lib/linux/${MES_ARCH}-mes-${MES_LIBC_SUFFIX}/crt1.c
...
${SRCDIR}/stack_c_interpreter ${BINDIR}/tcc.sl -c -D HAVE_CONFIG_H=1 -I include -I include/linux/${MES_ARCH} -o ${LIBDIR}/crt1_s_i.o lib/linux/${MES_ARCH}-mes-${MES_LIBC_SUFFIX}/crt1.c
...
# libtcc1.a
mkdir ${LIBDIR}/tcc
${TTC_G_BOOT} -c -D HAVE_CONFIG_H=1 -D HAVE_LONG_LONG=${HAVE_LONG_LONG} -D HAVE_FLOAT=0 -I include -I include/linux/${MES_ARCH} -o libtcc1_g.o lib/libtcc1.c
${TCC_BOOT} -c -D HAVE_CONFIG_H=1 -D HAVE_LONG_LONG=${HAVE_LONG_LONG} -D HAVE_FLOAT=0 -I include -I include/linux/${MES_ARCH} -o libtcc1.o lib/libtcc1.c
...
# Checksums
if ${ORG_STAGE0}/match x${UPDATE_CHECKSUMS} xTrue; then
    ${ORG_STAGE0}/sha256sum -o ${pkg}.${ARCH}.checksums \
        ${BINDIR}/tcc_s \
        ${BINDIR}/tcc-boot0 \
        ${BINDIR}/tcc-boot1 \
        ${BINDIR}/tcc \
    ...
    ${ORG_STAGE0}/sha256sum -c ${pkg}.${ARCH}.checksums
fi
