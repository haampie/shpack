 ...
 * brks the exact size per call, so each calloc is one brk (~52 brk/command,
 * making kaem ~3-10x slower than dash). This copy reuses per-command token and
 * string pools, caches the envp array, and stops copying argv, driving
 * per-command brk to ~0. Output is byte-identical (gated on the musl libc.a
...
/*
 * MES-replacement optimization state (see file header).
 * Reusable per-command token + string pools, rewound at the start of every
 * collect_command, so a command's tokens never call malloc. Plus a cached
 * envp array rebuilt only when the environment changes, and reusable scratch
 * buffers for find_executable.
 *
 * The pools are arrays OF POINTERS (struct Token** / char**), each slot
 * allocated once at startup, and indexed as pool[i]. We deliberately avoid
 * struct-pointer arithmetic (pool + i): M2-Planet does not scale it the way it
 * scales the pointer-array indexing that kaem already uses everywhere
 * (argv[i], array[index]), so pool[i] is the one M2-Planet-safe idiom here.
 */
struct Token** tok_pool;  /* MAX_ARRAY pre-allocated Token nodes */
char** str_pool;          /* MAX_ARRAY pre-allocated MAX_STRING buffers */
int pool_tok_idx;
int pool_str_idx;
char** envp_cache;        /* last-built envp array */
int env_dirty;            /* TRUE when env changed since envp_cache built */
char* fe_trial;           /* find_executable scratch (the returned path) */
char* fe_mpath;           /* find_executable scratch (PATH copy) */
...
char* find_executable(char* name)
{
	...
	/* Reuse the scratch buffers (allocated once in main) instead of calloc
	 * per command. trial is the candidate path we return on success; the
	 * caller (the parent in _execute) uses it before the next call. */
	char* trial = fe_trial;
	...
	while(NULL != next)
	{
		...
		/* Check existence cheaply with access() instead of opening and
		 * reading the whole candidate binary on every command. */
		if(0 == access(trial, 0))
		{
			return trial;
		}
...
char** list_to_array(struct Token* s)
{
	...
	while(n != NULL)
	{
		...
		if(n->var == NULL)
		{
			/* It is a line (argv element): point straight at the existing
			 * value -- no per-element calloc/copy needed. */
			array[index] = n->value;
		}
		else
		{
			/* It is a var (envp element): build "var=value" */
			...
			/* Terminate so the reset below only clears the first byte. */
			...
			/* Reset element; the var branch terminates itself above so
			 * clearing the first byte is enough. */
			element[0] = 0;
		}
 ...
 * has changed since last time. Upstream rebuilt this (one calloc per env var)
 * on every single exec; the cache makes it ~free for the common case where a
 * script's environment is fixed across its commands. */
char** env_to_array()
{
	if(env_dirty)
	{
...
int _execute(FILE* script, char** argv)
{
	...
	/* Build envp once, in the parent, from the cache. The child inherits the
	 * pointer across fork, so we no longer rebuild the whole environment array
	 * on every exec. */
	envp = env_to_array();
...
{
	command_done = FALSE;
	/* Rewind the per-command pools: the previous command's tokens are done
	 * with (each command's tokens are fully executed before the next is
	 * collected, including the recursive collect_command calls in if_cmd), so
	 * we reuse the same memory instead of calloc-ing fresh nodes/buffers. */
	pool_tok_idx = 0;
	pool_str_idx = 0;
	/* Initialize token */
	struct Token* n;
...
int main(int argc, char** argv, char** envp)
{
	...
	/* Allocate the reusable per-command pools and scratch buffers once, up
	 * front, so the command loop never has to. Each pool is an array of
	 * MAX_ARRAY pre-allocated slots (Token nodes / MAX_STRING buffers). */
	int pool_i;
	tok_pool = calloc(MAX_ARRAY, sizeof(char*));
