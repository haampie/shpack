...
void handle_variables(char** argv, struct Token* n)
{
	...
	/*
	 * Fast path (MES-replacement optimization): a token with no '$' has
	 * nothing to substitute. Return immediately, leaving n->value untouched
	 * (it points at the caller's pooled buffer). This avoids two
	 * calloc(MAX_STRING) calls -- and thus two brk syscalls, since M2libc's
	 * malloc never frees -- for the common no-variable token.
	 */
