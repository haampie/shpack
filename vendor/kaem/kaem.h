...
/* Vendored/optimized for MES-replacement: bumped from upstream's 512 so a
 * single command can hold the ~1260-object musl `ar` line (tcc -ar recreates
 * the archive each call, so it cannot be split). Also sizes the per-command
 * token/string pools in kaem.c. */
